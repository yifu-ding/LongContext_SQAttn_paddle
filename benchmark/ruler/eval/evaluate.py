...
Running
```
python evaluate.py     --data_dir /path/to/your/prediction_jsonl_folder     --benchmark synthetic
```
"""
import argparse
import re
...
def run_evaluation_per_task(task_config: dict, predictions_file: str, verbose: int = 0):
    inputs, predicts, references, indices = get_pred_and_ref(
        predictions_file=predictions_file, task_config=task_config
    )
...
def write_evaluation(results: dict):
    ...
    dfs = [["Tasks"] + tasks, ["Score"] + score, ["Nulls"] + nulls]
...
def aggregate_chunk(folder):
    ...
    chunk_files = sorted(
        [file for file in jsonl_files if re.match(".*[^_]+-\\d+\\.jsonl", file)]
    )
