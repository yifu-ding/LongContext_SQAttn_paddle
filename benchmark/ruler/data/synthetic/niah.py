...
python niah.py     --save_dir=./     --save_name=niah_single     --tokenizer_path=tokenizer.model     --tokenizer_type=nemo     --max_seq_length=4096     --tokens_to_generate=128     --num_samples=10     --template="Some special magic {type_needle_v} are hidden within the following text. Make sure to memorize it. I will quiz you about the {type_needle_v} afterwards.
{context}
...
def generate_input_output(num_haystack):
    ...
    for _ in range(args.num_needle_k):
        ...
        for _ in range(args.num_needle_v):
            ...
            needles.append(
                needle.format(
                    type_needle_v=args.type_needle_v, key=keys[-1], value=value[-1]
                )
            )
        values.append(value)
    ...
    input_text = template.format(
        type_needle_v=type_needle_v, context=context, query=query
    )
