...
python variable_tracking.py       --save_dir=./     --save_name=vt     --tokenizer_path=tokenizer.model     --tokenizer_type nemo     --max_seq_length 4096     --tokens_to_generate 30     --num_samples 10     --random_seed 42      --num_chains 1  --num_hops 4     --template "[INST] Memorize and track the chain(s) of variable assignment hidden in the following text.
...
"""
    ...
    if len(sentences) <= len(chains[0]):
        ...
        try:
            ...
            print("reduces chain length for not enough noises", flush=True)
...
def sys_vartrack_w_noise_random(
    ...
    while total_tokens + tokens_to_generate + example_tokens < max_seq_length:
        ...
        print(
            f"Current length {total_tokens + tokens_to_generate + example_tokens} | Max length {max_seq_length} | Noises: {num_noises}",
            flush=True,
        )
        if total_tokens + tokens_to_generate + example_tokens > max_seq_length:
            ...
            break
        num_noises += incremental
    print("Num noises:", num_noises, flush=True)
